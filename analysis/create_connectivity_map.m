...
%%% REQUIRES: existing bayes structure created with bayes_backwards_select
% by default, searches bayes, allows user to make selection
...
%%% to look up raw numbers, for the drug-drug data use
%%% symmetric_table

%%% to look up raw numbers for the moa data, use 
...
[bayes_file,path] = uigetfile('./bayes/*.mat');
...
%% create some colormaps for plotting 
% new map - bw 
...
variables_to_create = {'red_colormap', 'large_group', 'efflux_in_lipid','use_drug_full_name','crossval_mode'};
default_values =      [true,            true,            true,              true,              false];
...
% quick fix to move other to the bottom to make it easier with applied
% things
results_table.NEIGHBOR_MOA = cellstr(strrep(results_table.NEIGHBOR_MOA,'other','zzz'));
...
results_table.MOA  = cellstr(strrep(results_table.MOA,'other','zzz'));
...
moa_table.MOA = cellstr(strrep(moa_table.MOA,'other','zzz'));
...
% create heatmap -- skipping this for now
% while false 
%     if ~crossval_mode
...
%% make a clustergram for symmetric table 

%while false 
    if ~crossval_mode
        distance_metric = 'cosine';
        cluster_drug = clustergram(symmetric_table{:,:},'Colormap',parula_plus,...
            'DisplayRange',max(max(symmetric_table{:,:})),'Symmetric',false,...
            'RowLabels',symmetric_table.Properties.RowNames,'ColumnLabels',...
            symmetric_table.Properties.VariableNames,...
            'ColumnPDist',distance_metric,'RowPDist',distance_metric);
    end 
%end 
...
if ~crossval_mode
    connectivity_plot(bidirectional_table2,color_struct,all_categories,red_map,wbg)
...
bidirectional_table.NEIGHBOR_MOA = cellstr(strrep(bidirectional_table.NEIGHBOR_MOA,'other','zzz'));
...
try 
    ...
    disp("need more than one MoA category to calculate all connection sums")
    
end 

%% make moa plot clustergram
% just gonna turn this off for now
while false 
    if ~crossval_mode
        distance_metric = 'cosine';
        cluster_moa = clustergram(two_way_table_bi_moa_pct{:,:},'Colormap',parula_plus,...
            'DisplayRange',max(max(two_way_table_bi_moa_pct{:,:})),'Symmetric',false,...
            'RowLabels',two_way_table_bi_moa_pct.Properties.RowNames,'ColumnLabels',...
            two_way_table_bi_moa_pct.Properties.VariableNames,...
            'ColumnPDist',distance_metric,'RowPDist',distance_metric);
    end 
end 
...
drug_moas = cellstr(strrep(drug_moas,"other","zzz"));
...
return 
% just want this part below to be optional if ur feelin it
%% do work with symmetric_table making knn
% symmetric_table is what's used to make the drug drug nn

distance_metrics = ["euclidean","seuclidean","cityblock","chebychev",...
    "minkowski","cosine","correlation","spearman","hamming","jaccard"];

% the drugs would just be knn_drugs

knn_drugs = symmetric_table.Properties.VariableNames';

knn_data = symmetric_table{:,:}; 
...
%vert_table = two_way_table_bi_moa_pct'

for d_metric = distance_metrics

knn_drugs = two_way_table_bi_moa_pct.Properties.VariableNames';

knn_data = two_way_table_bi_moa_pct{:,:}';

%d_metric = 'cosine';

knn_helper

title(strcat("JP based off of MoA plot - ",d_metric))

savefig(knn_fig,strcat("./figures/2020-04-06 knn on heatmaps/MoA based/JP based on MoA heatmap ", d_metric))
end 