...
%                 'position',[20 80 560 510]);
  ...
  % S.LN.EdgeCData = zeros(length(knn_digraph.Edges.Weight),1);
    % make it pretty
    S.LN.MarkerSize = 5;
...
% callback function for the connectivity plot 
    ...
    % scaled distance - use for line thickness 
    scaled_distance = 7*knn_digraph.Edges.Weight./(max(knn_digraph.Edges.Weight));
    zero_inds = scaled_distance == 0;
    
    % want to get min nonzero value
    min_val = min(scaled_distance(scaled_distance>0));
    scaled_distance(zero_inds) = min_val;
